test_that("wavelet_decompose works correctly", {
  ...
  # Scale the test data to smaller values for numerical stability
  ...
  # Test reconstruction with relative tolerance
  rel_diff <- max(abs(test_data - recon)) / max(abs(test_data))
  expect_lt(rel_diff, 0.01)  # 1% relative difference tolerance
})
...
test_that("analyze_wavelet_variance works correctly", {
  set.seed(123)
  test_data <- rnorm(256)
  test_data <- test_data / max(abs(test_data))  # Scale data
